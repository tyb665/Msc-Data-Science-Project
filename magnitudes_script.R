...
simulate_b_models <- function(n = 5000, M0 = 2.5, alpha = 0.05, lambda = 4000) {
  ...
  b_models <- list(
    ...
    gaussian = {
      ...
      smooth_b <- as.numeric(scale(smooth_b)) * alpha + 1.0  # 标准化后加到均值1
...
plot_fitted_b <- function(result) {
  ...
  b_est <- exp(beta_mean + result$fit$summary.fixed$mean["Intercept"]) / log(10)
  
  # Real b(x) take the nearest point
  df_true <- result$df
  b_true_interp <- approx(x = df_true$x, y = df_true$b, xout = x_mesh)$y
  
  df_plot <- data.frame(x = x_mesh, b_est = b_est, b_true = b_true_interp)
  
  ggplot(df_plot, aes(x = x)) +
    geom_line(aes(y = b_est), color = "blue", size = 1.1) +
    geom_line(aes(y = b_true), color = "red", linetype = "dashed") +
    labs(title = paste("Model:", result$model),
         y = "b-value", x = "x") +
    theme_minimal()
}
...
## Plot the histogram of RMSE
library(ggplot2)

ggplot(rmse_table, aes(x = reorder(model, RMSE), y = RMSE, fill = model)) +
  geom_col() +
  labs(
    title = "RMSE of fitted b(x) by model type",
    x = "Model", y = "RMSE"
  ) +
  theme_minimal() +
